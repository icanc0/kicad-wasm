...
#include <sstream>
#include <streambuf>
// kicad-wasm: wxFile/wxFFile replaced with std::ifstream in GetFileContents.
// #include <wx/file.h>
// #include <wx/ffile.h>

// kicad-wasm: string_utils.h pulls the wxWidgets stack in. Not needed here.
...
namespace SEXPR
{
    ...
    {
        // kicad-wasm: use plain std::ifstream. Emscripten's virtual FS
        // handles the file path (mounted via preload-file or File API).
        std::ifstream in( aFileName, std::ios::binary );

        if( !in )
            throw PARSE_EXCEPTION( "Error occurred attempting to open file" );
        ...
        if( str.empty() )
            throw PARSE_EXCEPTION( "empty file" );
                ...
                if( closingPos != std::string::npos )
                {
                          ...
                          && tmp.find_first_not_of( "0123456789.", 1 ) == std::string::npos ) )
                    {
                        ...
                        if( tmp.find( '.' ) != std::string::npos )
                        {
                            ...
                            // kicad-wasm: std::strtod ("C" locale) instead of wxString::ToCDouble.
                            double fnumb = std::strtod( tmp.c_str(), nullptr );
